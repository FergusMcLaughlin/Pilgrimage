...
func _runTests() -> void:
	...
	await _runTest(_testQueueRejectsNullAndInvalidActions)
...
func _testModifyStatsThroughQueue() -> void:
	...
	var action := ActionType.make(
		...
		null,
		card,
		ModifyStatsPayload.create("health", 2, "test")
	)
...
func _testQueueRejectsNullAndInvalidActions() -> void:
	assert(!ActionQueue.enqueueAction(null), "The queue should reject a null action.")
	var invalidAction = GameAction.create("invalid_action", null, null)
	assert(!ActionQueue.enqueueAction(invalidAction), "The queue should reject an invalid action type.")
	assert(ActionQueue.getActionQueueSize() == 0, "Rejected actions should not change the queue.")
...
func _testMalformedModifyDoesNotWedgeQueue() -> void:
	...
	var action := ActionType.make(
		...
		null,
		card,
		ModifyStatsPayload.create("not_a_stat", 2, "test")
	)
...
func _testNegativeHealthModificationDealsDamage() -> void:
	...
	var action := ActionType.make(
		...
		null,
		card,
		ModifyStatsPayload.create("health", -(card.health + 10), "test")
	)
...
func _testDeleteCardOutsideActivePlay() -> void:
	...
	assert(is_instance_valid(card), "DELETE_CARD should not free a card that cannot be removed from active play.")
	card.queue_free()
...
func _testTwoActionsKeepFifoOrder() -> void:
	...
	var recordPopped = func(action: GameAction) -> void:
		if action.type == ActionType.MODIFY_STATS:
			var payload = action.payload as ModifyStatsPayload
			poppedAmounts.append(payload.amount)
	
	GlobalSignalBus.actionPopped.connect(recordPopped)
	ActionQueue.enqueueAction(ActionType.make(
		ActionType.MODIFY_STATS, null, card, ModifyStatsPayload.create("attack", 3, "test")
	))
	ActionQueue.enqueueAction(ActionType.make(
		ActionType.MODIFY_STATS, null, card, ModifyStatsPayload.create("attack", -1, "test")
	))
	await _waitForProcessor()
...
func _testUnsupportedActionDoesNotBlockNextAction() -> void:
	...
	assert(ActionQueue.enqueueAction(ActionType.make(
		ActionType.MODIFY_STATS, null, card, ModifyStatsPayload.create("attack", 2, "test")
	)))
	await _waitForProcessor()
...
func _testRemoveMovesCardToGraveyard() -> void:
	...
	assert(ActionQueue.enqueueAction(ActionType.make(
		...
		null,
		card,
		RemoveCardPayload.create(0, "test")
	)))
	await _waitForProcessor()
	await get_tree().process_frame
...
func _testRevealBlocksUntilAnimationFinishes() -> void:
	...
	var recordResolved = func(action: GameAction, result: Variant) -> void:
		if action.type == ActionType.REVEAL_CARD:
